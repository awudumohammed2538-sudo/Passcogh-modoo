...
app.use((req, res, next) => {
  ...
  if (origin && allowed.includes(origin)) res.setHeader('Vary', 'Origin');
...
async function initDb() {
  if (!pool) {
    console.warn('DATABASE_URL not set: using non-persistent development memory store. Configure PostgreSQL on Render for production.');
    return;
  }
  try {
  ...
  CREATE INDEX IF NOT EXISTS payments_user_idx ON payments(user_id); CREATE INDEX IF NOT EXISTS reading_user_idx ON reading_sessions(user_id);
    ...
    console.error('PostgreSQL initialization failed; keeping API online so /api/health can report the problem:', e);
  }
}
...
/* ----------------------------- HEALTH / CURRICULUM ----------------------------- */
app.get('/api/health', async (req,res)=>{const c=loadCurriculum(),ls=getLevels(),ss=allSubjects(),ts=ss.reduce((n,s)=>n+topics(s).length,0);let dbStatus='memory';let liveDbError=dbInitError;if(pool){try{await dbQuery('SELECT 1');dbStatus='postgresql';liveDbError=null;}catch(e){dbStatus='error';liveDbError=String(e?.message||e);}}res.json({success:true,service:'PASSCOGH-MODOO',version:c.version||'unknown',database:dbStatus,databaseConfigured:Boolean(pool),databaseError:liveDbError,curriculumFile:fs.existsSync(CURRICULUM_FILE),curriculumShape:Array.isArray(c.levels)?'levels[]':typeof c.levels,levels:ls.length,subjects:ss.length,topics:ts,diagrams:diagrams().length,skillsPath:Boolean(c.skills_path?.enabled),universityGuidance:Boolean(c.university_guidance?.enabled),freeOnlineReading:true,readingAdIntervalMinutes:READING_AD_MINUTES,jhsShsTopicPdfPriceGhs:JHS_SHS_PDF_PRICE_GHS,preUniversityCheckPriceGhs:PRE_UNI_CHECK_PRICE_GHS});});
function frontendCurriculumPayload() {
  const c = loadCurriculum();
  const raw = c && c.levels && typeof c.levels === 'object' ? c.levels : {};
  const cloneLevel = (id) => {
    const v = raw[id];
    return v && typeof v === 'object' ? { id, ...v } : { id, name:id, subjects:[] };
  };
  const mergeLevels = (ids, label) => {
    const bySubject = new Map();
    for (const id of ids) {
      const level = cloneLevel(id);
      const subjectList = Array.isArray(level.subjects) ? level.subjects : [];
      for (const subject of subjectList) {
        if (!subject || typeof subject !== 'object') continue;
        const key = norm(subject.code || subject.id || subject.name || subject.title);
        if (!key) continue;
        if (!bySubject.has(key)) {
          bySubject.set(key, {
            ...subject,
            id: subject.id || subject.code || idNorm(subject.name || subject.title || key),
            name: subject.name || subject.title || subject.id || key,
            level: label,
            topics: []
          });
        }
        ...
        for (const topic of sourceTopics) {
          ...
          target.topics.push({ ...topic, curriculumLevel: id });
        }
      }
    }
  ...
  return {
    ...
    success: true,
    curriculum: c,
    "pre-shs": cloneLevel("pre-shs"),
    jhs: { id:"jhs", name:"JHS 1–3", description:"JHS 1–3 curriculum", subjects:mergeLevels(["jhs1","jhs2","jhs3"],"JHS 1–3") },
    shs: { id:"shs", name:"SHS 1–3", description:"SHS 1–3 curriculum", subjects:mergeLevels(["shs1","shs2","shs3"],"SHS 1–3") },
    "pre-uni": cloneLevel("pre-university"),
    tertiary: cloneLevel("university-college")
  };
}
app.get('/api/curriculum',(req,res)=>res.json(frontendCurriculumPayload()));
...
initDb().finally(()=>app.listen(PORT,HOST,()=>{const c=loadCurriculum(),ls=getLevels(),ss=allSubjects();console.log(`PASSCOGH-MODOO listening on ${HOST}:${PORT}`);console.log(`Curriculum: ${fs.existsSync(CURRICULUM_FILE)?'FOUND':'MISSING'} | levels=${ls.length} subjects=${ss.length} topics=${ss.reduce((n,s)=>n+topics(s).length,0)}`);console.log(`Diagram library: ${diagrams().length}`);console.log(`PostgreSQL: ${pool?'ENABLED':'NOT CONFIGURED'}${dbInitError?' (initialization error reported by /api/health)':''}`);console.log(`Paystack: ${PAYSTACK_SECRET_KEY?'CONFIGURED':'NOT CONFIGURED'}`); }));
